    ...
    try:
        ...
        if response_base_url.status == 200:
            ...
            if response_event_url.status == 200:
                ...
                bmc_time = response_managers_members.dict["DateTime"]
                dt = datetime.datetime.strptime(bmc_time, "%Y-%m-%dT%H:%M:%S+08:00")
                timestamp = (dt+(datetime.timedelta(minutes=2))).strftime("%Y-%m-%dT%H:%M:%S+08:00")
