...
def lenovo_https_certificate_import(ip, login_account, login_password, certfile):
    ...
    try:
        ...
        if "VerifyRemoteServerCertificate" in response_update_service_url.dict and "RemoteServerCertificates" in response_update_service_url.dict:
            enable_flag = False
            ...
            if response_upload_url.status == 201:
                if enable_flag is True:
                    print("Enable verify of the HTTPS file server successfully.")
                result = {'ret':True,
                ...
                result = {"ret": False,
                          "msg": "Upload certificate failed. The current number of certificates in the target certificate collection already reached the maximum mumber:4."}
                return result
            else:
...
def add_helpmessage(parser):
    parser.add_argument('--certfile', type=str, required=True, help="An file that contains signed certificate in PEM format. Note: SR635/SR655 is not support upload HTTPS certificate.")
