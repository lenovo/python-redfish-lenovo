    ...
    for count in range(chassis_count):
        ...
        if response_chassis_url.status == 200:
            ...
            if response_nic_adapter_url.status == 200:
                ...
                result = {'ret': False,'msg': "response nic adapter url Error code %s" % response_nic_adapter_url.status}
                REDFISH_OBJ.logout()
                return result
            nic = 0
            for nic in range(0, nic_adapter_count):
                ...
                if response_nic_adapter_x_url.status == 200:
                    ...
                    result = {'ret': False, 'msg': "response nic_adapter_x_url Error code %s" % response_nic_adapter_x_url.status}
                    REDFISH_OBJ.logout()
                    return result
                port = 0
                ...
                for dev in range(0, nic_dev_count):
                    ...
                    if response_nic_dev_x_url.status == 200:
                        ...
                        nic_details.append(NIC_Devices)
                        ...
                        if response_nic_port_x_url.status == 200:
                            ...
                            nic_details.append(Physical_Ports)
                        else:
                            result = {'ret': False, 'msg': "response nic_port_x_url Error code %s" % response_nic_port_x_url.status}
                            REDFISH_OBJ.logout()
                            return result
                    else:
                        result = {'ret': False, 'msg': "response nic_dev_x_url Error code %s" % response_nic_dev_url.status}
                        REDFISH_OBJ.logout()
                        return result
        else:
